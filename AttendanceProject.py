...
def findEncodings(images):
    encodeList = []
    for img in images:
        imgBasic = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
        encode = face_recognition.face_encodings(imgBasic)[0]
        encodeList.append(encode)
    return encodeList
def markAttendance(name):
    with open('AttendanceDB.csv','r+') as f:
...
while True:
    ...
    encode = face_recognition.face_encodings(imgS,facesCurr)

    for encodeFace,faceLoc in zip(encode,facesCurr):
        matches = face_recognition.compare_faces(encodeListKnown,encodeFace)
        faceDistance = face_recognition.face_distance(encodeListKnown,encodeFace)
        # print(faceDistance,matches)
